...
- Q: When the active model is removed, which fallback applies? → A: Use the product's configured default model when one exists; otherwise the first model in the ordering defined in FR-013.
...
A user can define a new model that is not already in their saved list by providing a stable machine-oriented identifier (for example, a provider path), a short human-readable title, and selecting a provider from the supported list. They confirm through an explicit save action in a dedicated step (such as a dialog). The new entry appears in the selectable list and remains available after closing and reopening the application on the same device.
...
1. **Given** the user has entered a new identifier, title, and selected a provider, **When** they confirm save, **Then** the model appears in the list and is persisted for future sessions on this device.
...
A user can remove a previously saved custom model through a clear delete affordance on the Models page. After removal, the list updates immediately. If the removed entry was the active selection, the system selects a sensible fallback (another available model or a default from the app) so chat remains usable.
...
2. **Given** the active model is a custom entry, **When** the user removes that entry, **Then** the app switches to the fallback defined in FR-012 without leaving chat in a broken state when any model remains selectable.
...
- No saved models yet: dropdown is empty; add flow remains reachable from the Models page.
...
- **FR-002**: Users MUST be able to reach a dedicated Models page (accessible from the main navigation) to add, view, and delete saved models.
- **FR-003**: The Models page MUST expose a provider selector. The product ships a fixed `SUPPORTED_PROVIDERS` list; users choose from this list only (no free-text entry). The first provider is selected by default.
- **FR-004**: The Models page MUST filter the displayed model list to show only entries belonging to the selected provider.
- **FR-005**: The add flow MUST collect a stable model identifier and a human-readable display title before saving. The provider is inherited from the currently selected provider on the Models page.
- **FR-006**: The system MUST validate that both identifier and title are non-empty before persisting a new entry.
- **FR-007**: The system MUST reject a new saved entry when its identifier duplicates another saved entry after normalization. Normalization: trim surrounding whitespace; comparisons are case-insensitive.
- **FR-008**: User-saved model entries MUST persist across application restarts on the same device until removed.
- **FR-009**: When the chat experience loads, saved entries MUST be loaded so they appear in the selectable set without requiring a manual refresh.
- **FR-010**: Users MUST be able to remove a saved custom model via an explicit delete control on the Models page.
- **FR-011**: After add or remove on the Models page, the displayed list MUST update immediately to reflect the current set of saved entries.
- **FR-012**: If the currently active model is removed, the system MUST set the active model to the first model in the order defined in FR-013. If no models remain in the selectable set, the product MUST surface an explicit state consistent with Edge Cases.
- **FR-013**: The selectable list MUST show user-saved models sorted alphabetically by display title using case-insensitive ordering.
- **FR-014**: The product MAY show brief non-blocking confirmation when an entry is added or removed; if not used, the updated list itself MUST make the outcome obvious.

### Key Entities *(include if feature involves data)*

- **Model entry (saved)**: A user-defined record with a unique identifier string (unique among saved entries after normalization), a display title for lists, and a provider drawn from `SUPPORTED_PROVIDERS`.
- **SUPPORTED_PROVIDERS**: A product-defined, fixed list of provider values (e.g. `openrouter`). Users select from this list; free-text provider entry is not supported.
- **Selectable model set**: All user-saved entries presented in the chat UI for the current device, ordered per FR-013.
...
- Duplicate detection uses the normalization rules in FR-007.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A typical user can add a new custom model (open Models page, enter required fields, save, see it listed) in under two minutes on first attempt without documentation.
...
- **SC-003**: In structured testing, 100% of attempts to save a duplicate identifier (among saved entries, per FR-007) are rejected with no duplicate row created.

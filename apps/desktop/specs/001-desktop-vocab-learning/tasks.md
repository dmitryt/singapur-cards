...
- [ ] T013 [P] Align `specs/001-desktop-vocab-learning/contracts/tauri-commands.md` with backend-owned aggregated `get_entry_detail` behavior and MVP `search_entries` match kinds (`exact`, `prefix`, `partial`), then create typed `invoke()` wrappers for all Tauri commands in `src/lib/tauri/commands.ts`
...
**Independent Test**: Import a valid DSL file → dictionary appears in list with correct language pair and non-zero `entryCount` → search finds exact + prefix + partial matches → opening a result renders aggregated entry detail for that headword — all without network access.
...
- [ ] T022 [US1] Implement `search_entries` command in `src-tauri/src/commands/dictionary.rs` — FTS5 query on `normalized_headword`, filter by `searchLanguage` / `dictionaryIds`, return `matchKind` (`exact` / `prefix` / `partial`) for the MVP, respect `limit` (depends on T011)
- [ ] T023 [US1] Implement backend-owned aggregated `get_entry_detail` command in `src-tauri/src/commands/dictionary.rs` — accept a selected `entryId`, resolve sibling entries that share the selected headword + `languageFrom`, and return structured combined detail in one response (depends on T011)
- [ ] T024 [US1] Write Rust DB integration tests for import + search in `src-tauri/tests/db_integration_tests.rs` — round-trip on temp SQLite: import → list → search → aggregated `get_entry_detail` (depends on T019, T022, T023)

### Frontend — US1

- [ ] T025 [P] [US1] Create `dictionarySlice.ts` in `src/store/slices/dictionarySlice.ts` — state: `dictionaries[]`, `importProgress`, `importStatus`, `searchQuery`, `searchLanguage`, `searchResults`, `selectedEntry`; actions: `loadDictionaries`, `importDictionary`, `removeDictionary`, `searchEntries`, `getEntryDetail`
...
- [ ] T031 [US1] Build `SearchPage.tsx` in `src/pages/SearchPage.tsx` — composes `DictionaryManager`, `SearchBar`, `SearchResultList`, and entry detail panel; when a result is selected, call aggregated `get_entry_detail` once and render the backend-combined translations in the panel (no client-side fan-out); wire to `dictionarySlice` (depends on T027–T030)
- [ ] T032 [US1] Wire `SearchPage` into `AppShell` routing in `src/App.tsx`

**Checkpoint**: User Story 1 fully functional — import → progress → search → entry detail, offline, persists across restarts. Run `cargo test` (T017, T024) before advancing.
...
- [ ] T033 [P] [US2] Implement `create_card_from_entry` command in `src-tauri/src/commands/cards.rs` and supporting card insert/membership query helpers in `src-tauri/src/db/queries.rs` — inserts `Card` row, returns `CreateCardFromEntryOutput`; on duplicate `source_entry_id` returns `ConflictFailure` with `existingCardId`
...
- [ ] T041 [US2] Wire "Save as Card" action from entry detail in `SearchPage` — calls `create_card_from_entry`; on `ConflictFailure` navigate to `existingCardId` card (depends on T033, T039)
...
- **US2 (P2)**: Depends on US1 entry detail flow for the primary "save from dictionary" path; backend card CRUD can start once Phase 2 is complete
...
4. **STOP and VALIDATE**: import a real DSL file, search for words, open entry detail offline
...
- Aggregated dictionary entry detail is owned by the backend command surface; the frontend should make a single detail request per selected result
- MVP search stops at `exact`, `prefix`, and `partial` matching; true fuzzy search is deferred until a concrete algorithm and perf budget are agreed

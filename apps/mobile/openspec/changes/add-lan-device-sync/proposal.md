...
- Add **desktop-led pairing** using a QR code or manually entered connection details plus pairing code
...
### New Capabilities
- `device-sync-pairing`: Pair a mobile device with a desktop app on the same LAN using QR scan or manual entry, then persist the trust relationship for later sync sessions
...
- `src/app/` — new pairing and sync-management UI, including QR scan/manual entry and sync status
